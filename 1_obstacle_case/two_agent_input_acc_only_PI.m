...
%         eps_t_all_4 = randn(1, runs); %GPU array that stores eps_4(t) at the start of each sample path starting at time t and state xt
        ...
        S_tau_all = arrayfun(@simulateMC_input_acc_2agents_only_PI, eps_t_all_1, eps_t_all_2, eps_t_all_3, eps_t_all_4, ...
            ...
            t*ones(1,runs), h*ones(1,runs), T*ones(1,runs), b*ones(1,runs), e*ones(1,runs), s*ones(1,runs), cx*ones(1,runs), cy*ones(1,runs), r*ones(1,runs), pgx*ones(1,runs), pgy*ones(1,runs), ell*ones(1,runs), eta1*ones(1,runs), eta2*ones(1,runs));
